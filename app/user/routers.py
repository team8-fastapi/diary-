...
def user_login_handler(body: UserLoginRequest):
    ...
    try:
        ...
        # 5) DB에 클라이언트 토큰 저장
        session.add(RefreshToken(user_id=user.id, token=refresh_token, expires_at=expires_at, used=False))
        session.commit()
...
def user_logout_handler(request: Request):
    ...
    try:
        ...
        token_obj = session.query(RefreshToken).filter_by(token=refresh_token, used=False).first()
        if not token_obj:
            raise HTTPException(status_code=400, detail="Invalid or already logged out refresh token")
